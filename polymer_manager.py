...
if __name__ == "__main__":
    ...
    poly1.save("params.top", format="gromacs", overwrite=True)
    poly1.save("params.gro", format="gro", overwrite=True)
    poly1.save("params.pdb", format="pdb", overwrite=True, renumber=False)
    poly2 = polymer.get_structure_noparams()
    # print(poly2.atoms, poly2.residues)
    poly2.save("noparams.pdb", format="pdb", overwrite=True, renumber=False)
    # poly2.save(f"new-{pdb_path.name}.gro", format="gro", overwrite=True)
    new_name = Path(pdb.replace("_NEW", "")).stem
    with open(f"NAMED-{new_name}.pdb", "w") as f:
        # sort your atoms globally however you like:
        sorted_atoms = sorted(poly2.atoms, key=lambda a: a._idx)
