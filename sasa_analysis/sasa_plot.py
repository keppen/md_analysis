...
if __name__ == "__main__":
    ...
    glob_avg_chcl3 = f"analysis/boc-*/sasa_{title}.csv"

    files_chcl3 = sorted(base_dir.glob(glob_avg_chcl3))

    glob_avg_acn = f"analysis-acn/boc-*/sasa_{title}.csv"
    ...
    def parse_system(name):
        stereo = name.split("(")[1].split(")")[0]
        solvent = name.split(",")[-1].strip()
        return stereo, solvent

    order = ["SSSS", "SSSR", "SRSS", "SRSR"]

    data = {stereo: {} for stereo in order}

    for name in system_names:
        if "Pg4" not in name:
            continue

        stereo, solvent = parse_system(name)
        if stereo in order and solvent in ["CHCl3", "ACN"]:
            data[stereo][solvent] = df.loc[name]

    means_chcl3 = [data[s]["CHCl3"]["Mean"] for s in order]
    stds_chcl3 = [data[s]["CHCl3"]["Std"] for s in order]

    means_acn = [data[s]["ACN"]["Mean"] for s in order]
    stds_acn = [data[s]["ACN"]["Std"] for s in order]
    ...
    plt.bar(x - width / 2, means_chcl3, width, yerr=stds_chcl3, label="CHCl3")
    plt.bar(x + width / 2, means_acn, width, yerr=stds_acn, label="ACN")

    plt.xticks(x, order)
    plt.ylabel("SASA (Å^2)")
    plt.title(f"SASA per System: {title}")
    # plt.ylim([600, 1400])
    plt.legend()
